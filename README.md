...
Note: The theory does not specify how $g_h \sim \mathcal{G}_h$ has to be chosen; this is part of future work. The idea, however, is that the robot doesn't know and doesn't try to guess the humans' concrete goals but has a feasible idea of all possible goals.

Besides that, we have the normative parameters of the theory $\gamma_r, \beta_r, \gamma_h, \zeta, \xi, \eta$ shaping the solution, and $U_h(s,g_h) = [s \in g_h]$ is the indicator function.
...
$$
V_h(s,g_h) \gets \mathbb{E}_{a_r \sim \pi_r(s)} \mathbb{E}_{g_{-h}} \mathbb{E}_{a_\mathcal{H} \sim \pi_\mathcal{H}(s,g)} \mathbb{E}_{s' \sim T(s,a)} [U_h(s',g_h) + (1 - U_h(s',g_h)) \gamma_h V_h(s', g_h)]
\tag{6}
$$
...
$V_h(\cdot, g_h)$ is constant on that trajectory since $V_h(s_i,g_h) = V_h(s_{i+1},g_h)$ for all non-terminal states. Then $X_h$ and $U_r$ are also constant on the trajectory. Since $V_r(s_i) = U_r(s_T) + V_r(s_{i+1})$, we get $V_r(s_i) = (T - i + 1) \cdot U_r(s_T)$.

Note that $U_r < 0$ and values closer to zero are better. So a policy that creates shorter trajectories performs better. If all episodes have equal length then relative performance between policies only depends on $U_r(s_T)$.
...
There is another situation where we might be able to use some form of backwards induction even in a cyclic environment, but this is less relevant. I just keep it here for completeness' sake.
We need
- all goals $g_h$ consist of terminal states
- $\gamma_r = \gamma_h = 1$

In this case $U_r(s)$ only depends on the terminal state reached by the policy and the length of the trajectory created by the policy. So for a backwards induction we would need to take length into account.
